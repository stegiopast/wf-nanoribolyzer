...
def create_intensity_matrix(fusion_alignment_df):
    ...
    id_dict.keys()
    dbscan_matrix = np.array([[x,y,intensity_matrix[x,y]] for x,y in zip(*np.nonzero(intensity_matrix))])
...
def create_colored_bed(table_name = "", output_folder = output, output_file = "", sample_type = ""):
    ...
    bed_df = over_mean_df[["ID","Refstart","Refend","ID","Score","Strand","Thikstart","Thikend","Colors"]]
    bed_df.iloc[:,0] = "RNA45SN1"
...
def intensity_fusion(fusion_alignment_df = pd.DataFrame()):
