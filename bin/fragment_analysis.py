...
opt_parser.add_argument(
    ...
    "--threshold",
    dest="threshold",
    help="Insert a overlap Threshold between 0-1 (1 -> 100 percent overlap)",
    metavar="FILE",
)
opt_parser.add_argument(
...
def size_bin_selection(
    template_length, reduce_gt_1000=100, reduce_gt_100=50, reduce_gt_10=25
):
    ...
    while temporary_length > 5:
        if temporary_length > 1000:
            list_of_lengths.append(
                [temporary_length - 1, (temporary_length - reduce_gt_1000)]
            )
            temporary_length -= reduce_gt_1000
        elif temporary_length > 100:
            list_of_lengths.append(
                [temporary_length - 1, (temporary_length - reduce_gt_100)]
            )
            temporary_length -= reduce_gt_100
        else:
            list_of_lengths.append(
                [temporary_length - 1, (temporary_length - reduce_gt_10)]
            )
...
def pearson_correlation(_temp_alignment_df_path, _reference_dict):
    ...
    if _temp_alignment_df != "[]":
        ...
        if _temp_alignment_df.shape[0] > 2:
            pearson_corr_matrix = [
                [0 for j in range(_temp_alignment_df.shape[0])]
                for i in range(len(_reference_dict["Sequence"]) + 1)
            ]
            temp_iterator = 0
            for row, rowstart, rowend in zip(
                _temp_alignment_df["Sequence"],
                _temp_alignment_df["Refstart"],
                _temp_alignment_df["Refend"],
            ):
            ...
            pearson_df = pd.DataFrame(
                pearson_corr_matrix, columns=list(_temp_alignment_df.index)
            )
            pearson_corr_result_df = pearson_df.corr()
            pearson_corr_result_df = pearson_corr_result_df.iloc[
                0 : pearson_corr_result_df.shape[0] + 1,
                0 : pearson_corr_result_df.shape[1] + 1,
            ]
            ...
            non_fusion_candidates_list = [
                list(np.unique(np.array(i)))
                for i in temp_fusion_candidates_list
                if len(i) < 2
            ]
            ...
            for non_fusion_group_list in non_fusion_candidates_list:
                ...
                temp_single_reads_df = pd.concat(
                    [temp_single_reads_df, non_fusion_df], axis=0
                )
        else:
            temp_single_reads_df = pd.concat(
                [temp_single_reads_df, _temp_alignment_df], axis=0
            )
...
def fusion_read_groups(_read_group_path, _reference_dict):
    ...
    if temp_df != "[]":
        ...
        position_array = [
            ...
            for location in range(_reference_dict["Length"])
        ]
        for row_seq, row_prop_seq, rowstart, rowend, row_n_reads in zip(
            temp_df["Sequence"],
            temp_df["Proportion_Sequence"],
            temp_df["Refstart"],
            temp_df["Refend"],
            temp_df["n_Reads"],
        ):
            if row_prop_seq == []:
                for j in range(_reference_dict["Length"]):
                    if j >= rowstart and j < rowend:
                        ...
                        position_array[j][nucleotide] = (
                            position_array[j][nucleotide] + row_n_reads
                        )
                    else:
                ...
                for j in range(_reference_dict["Length"]):
                    if j >= rowstart and j < rowend:
                        position_array[j]["A"] = (
                            position_array[j]["A"] + row_prop_seq[j - rowstart][0]
                        )
                        position_array[j]["T"] = (
                            position_array[j]["T"] + row_prop_seq[j - rowstart][1]
                        )
                        position_array[j]["C"] = (
                            position_array[j]["C"] + row_prop_seq[j - rowstart][2]
                        )
                        position_array[j]["G"] = (
                            position_array[j]["G"] + row_prop_seq[j - rowstart][3]
                        )
                        position_array[j]["U"] = (
                            position_array[j]["U"] + row_prop_seq[j - rowstart][4]
                        )
                        position_array[j]["-"] = (
                            position_array[j]["-"] + row_prop_seq[j - rowstart][5]
                        )
                    else:
                        position_array[j]["-"] = position_array[j]["-"] + row_n_reads
        string = ""
        ...
        temp_string = (
            "-" * min_refstart + string + "-" * (_reference_dict["Length"] - max_refend)
        )
        ...
        output_dict = {
            "ID": temp_df["ID"][0],
            "Sequence": string,
            "Proportion_Sequence": absolute_base_count_array[min_refstart:max_refend],
...
def percentage_of_fits_parrallel(
    index, alignment_df_row, reference_length, reference_sequence
):
    row_sequence = (
        "-" * alignment_df_row[1]
        + alignment_df_row[0]
...
def create_colored_bed(
    table_name="", output_folder=output, output_file="", sample_type=""
):
...
def pearson_correlation_fusion(
    pearson_alignment_df=pd.DataFrame(),
    iterator=0,
    batch_size=100,
    overlap_threshold=overlap_threshold,
    reduce_size_bins_gt_1000=50,
    reduce_size_bins_gt_100=25,
    reduce_size_bins_gt_10=10,
):
    if iterator == 2:
        print("Final print")
        ...
        list_of_lengths = size_bin_selection(
            template_length,
            reduce_size_bins_gt_1000,
            reduce_size_bins_gt_100,
            reduce_size_bins_gt_10,
        )
        ...
        for length_segment in tqdm(list_of_lengths, total=len(list_of_lengths)):
            ...
            temp_alignment_df = pearson_alignment_df.loc[
                pearson_alignment_df["Length"] <= maximum_length, :
            ]
            temp_alignment_df = (
                temp_alignment_df.loc[temp_alignment_df["Length"] >= minimum_length, :]
                .sort_values(by=["Refstart", "Length"], ascending=[True, False])
            ...
            if temp_alignment_df.shape[0] > 0:
                if not temp_alignment_df.shape[0] > batch_size:
                    ...
                    with open(
                        f"{output}temp_parallel_comp_{length_segment[0]}_{length_segment[1]}_small.json",
                        "w",
                    ) as json_file:
                        json.dump(temp_alignment_json, json_file)
                    out_list_read_groups, out_list_read_singles, out_single_reads_df = (
                        pearson_correlation(
                            f"{output}temp_parallel_comp_{length_segment[0]}_{length_segment[1]}_small.json",
                            reference_dict,
                        )
                    ...
                    if out_single_reads_df.shape[0] != 0:
                        single_reads_df = pd.concat(
                            [single_reads_df, out_single_reads_df], axis=0
                        )
                else:
                    ...
                    for index, k in enumerate(bins):
                        if k < length:
                            k_1 = k * batch_size
                            k_2 = (k + 1) * batch_size
                            df_for_parallel_comp = temp_alignment_df.iloc[
                                k * batch_size : ((k + 1) * batch_size), :
                            ]
                            df_for_parallel_comp_json = df_for_parallel_comp.to_dict(
                                orient="records"
                            )
                            with open(
                                f"{output}temp_parallel_comp_{length_segment[0]}_{length_segment[1]}_{k_1}_{k_2}_{index}_big.json",
                                "w",
                            ) as json_file:
                            ...
                            df_for_parallel_comp = temp_alignment_df.iloc[
                                k * batch_size : len(temp_alignment_df), :
                            ]
                            df_for_parallel_comp_json = df_for_parallel_comp.to_dict(
                                orient="records"
                            )
                            with open(
                                ...
                                json.dump(df_for_parallel_comp_json, json_file)
                        list_for_parallel_comp.append(
                            f"{output}temp_parallel_comp_{length_segment[0]}_{length_segment[1]}_{k_1}_{k_2}_{index}_big.json"
                        )
                    with Pool(cores) as p:
                        pool_output = p.starmap(
                            pearson_correlation,
                            zip(list_for_parallel_comp, repeat(reference_dict)),
                        )
                        list_for_parallel_comp = []
                    for index, objects in enumerate(pool_output):
                        for index2, read_group in enumerate(objects[0]):
                            list_read_groups.append(read_group)
                        single_reads_df = pd.concat(
                            [single_reads_df, objects[2]], axis=0
                        )
                    p.close()
        ...
        with Pool(cores) as p:
            pool_output = p.starmap(
                fusion_read_groups,
                zip([path for path in list_read_groups], repeat(reference_dict)),
            )
        ...
        for idx, row in tqdm(
            single_reads_df.iterrows(), total=single_reads_df.shape[0]
        ):
            if row["ID"] != "RNA45SN1":
        ...
        summary_df = pearson_correlation_fusion(
            pearson_alignment_df=consensus_df,
            iterator=final_iterator,
            batch_size=new_batch_size,
            overlap_threshold=overlap_threshold,
            reduce_size_bins_gt_1000=100,
            reduce_size_bins_gt_100=50,
            reduce_size_bins_gt_10=25,
        )
        return summary_df
    ...
    # This filtering step filters out consensus sequences that to not fit the reference to a certain threshold in terms of sequence matches (default 90%)
        ...
        list_read_singles = []
        single_reads_df = pd.DataFrame()
        pearson_alignment_df = pearson_alignment_df.loc[
            ((pearson_alignment_df["ID"] != str(reference)))
        ]
        unclustered_df = pearson_alignment_df.loc[
            pearson_alignment_df["n_Reads"] == 1, :
        ]
        unclustered_df.to_csv(output + "unclustered_reads_df.csv", sep=";")
        if batch_size > 300:
            pearson_alignment_df = pearson_alignment_df.loc[
        ...
        out_list_read_groups, out_list_read_singles, out_single_reads_df = (
            pearson_correlation(
                f"{output}pearson_alignment_df_final.json", reference_dict
            )
        )
        for index, read_group in enumerate(out_list_read_groups):
            list_read_groups.append(read_group)
        ...
        final_consensus_df = final_consensus_df.sort_values(
            by=["Refstart", "Length"], ascending=[True, False]
        ).reset_index(drop=True)
        ...
        for i, row_sequence, rowstart, rowend in tqdm(
            zip(
                range(final_consensus_df.shape[0]),
                final_consensus_df["Sequence"],
                final_consensus_df["Refstart"],
                final_consensus_df["Refend"],
            ),
            total=final_consensus_df.shape[0],
        ):
            row_sequence = (
                "-" * rowstart
                + row_sequence
                + "-" * (reference_dict["Length"] - rowend)
            )
            if i == 0:
        ...
        final_consensus_df = final_consensus_df.loc[
            ((final_consensus_df["Percentage_of_fits"])) > 0.9, :
        ]
        final_consensus_df = final_consensus_df.sort_values(
            by=["Refstart", "Length"], ascending=[True, False]
        ).reset_index(drop=True)
        new_iterator = 2
        unclustered_df_last = final_consensus_df.loc[
            final_consensus_df["n_Reads"] == 1, :
        ]
        unclustered_df_last.to_csv(
            output + "unclustered_reads_df_after_last_lap.csv", sep=";"
        )
        final_consensus_df = final_consensus_df.loc[
            final_consensus_df["n_Reads"] != 1, :
        ]
        summary_df = pearson_correlation_fusion(
            pearson_alignment_df=final_consensus_df,
...
logger.info(
    "---                Create alignment with sequence and cigarstring of bam file         ---"
)
cigartuples = []
read_ids = []
read_sequences = []
...
for read in tqdm(samfile.fetch(), total=number_of_reads):
    read_data = {
        "ID": read.query_name,
        "Sequence": "-" * int(read.reference_end - read.reference_start),
        "Proportion_Sequence": [],
...
final_strings = []
for c_tuple_read, read_id, read_sequence, start_point_ref, end_point_ref in tqdm(
    zip(cigartuples, read_ids, read_sequences, read_starts, read_ends),
    total=len(read_ids),
):
    string = ""
    position_read = 0
...
logger.info(
    "---                Quality check for reference and query identity                     ---"
)
with Pool(cores) as p:
    pool_output = p.starmap(
        percentage_of_fits_parrallel,
        zip(indices_list, read_list_df, repeat(reference_len), repeat(reference_seq)),
    )
selected_indices = []
for objects in pool_output:
    if objects[0] >= 0:
...
logger.info(
    "---                      Perform pearson correlation and clustering                     ---"
)
...
create_colored_bed(
    table_name=output + "fragment_df.csv",
