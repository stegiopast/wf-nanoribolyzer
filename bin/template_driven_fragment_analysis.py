...
opt_parser.add_argument("-f", "--fragment_file",dest="fragment_name", help="Insert a fragment reference file", metavar="FILE")
...
opt_parser.add_argument("-s", "--sample_type",dest="sample_type", help="Mention which type of sample (Cytoplasm,Nucleus,SN1,SN2,SN3) you have or just name a colorset (blue,red,green,orange,purple)", metavar="FILE")
...
#                                                                                                                                   #
#####################################################################################################################################
def reconstruct_alignment(samfile):
    counter_forward = 0
    counter_reverse = 0
    temp_list = []
    for read in tqdm(samfile.fetch(),total=number_of_reads):
        ...
        for c_tuple in c_tuple_read: 
            ...
            if operation == 0:
                ...
                position_read += length
        read_data = {"ID" : read.query_name,"Sequence" : string,"Proportion_Sequence": [], "Length": int(read.reference_end - read.reference_start), "Refstart": read.reference_start, "Refend": read.reference_end, "n_Reads": 1,"IDS": [read.query_name]}
        temp_list.append(read_data)
    alignment_df = pl.DataFrame(temp_list)
    return alignment_df,counter_forward,counter_reverse

def percentage_of_fits_parrallel(index,alignment_df_row, reference_length, reference_sequence, identity):
...
def argmax_of_min_overlap_fragment_association_parallel(query_row_seq, query_row_refstart, query_row_refend,query_row_length, query_row_id, fragment_df):
    ...
    # The match score is calculated for all matches within the overlaping region.  
    # The argmax in the overlaps is taken as the best fitting fragment
    ...
    query_sequence = "*" * query_row_refstart + query_row_seq + "*" * (ref_data["Length"] - query_row_refend)
    for frag_row_seq, frag_row_start, frag_row_end, frag_row_length in zip(fragment_df["Sequence"],fragment_df["Start"],fragment_df["End"],fragment_df["Length"]):
        ...
        if min_end - max_start < 0:
            ...
            for base_in_query,base_in_frag in zip(query_sequence[max_start:min_end],frag_sequence[max_start:min_end]):
    ...
    return fitting_stats,fitting_stat_overlap,fitting_stat_matches, fitting_stat_fragment, query_row_id

def min_length_intersection_association(alignment_df,fragment_df):
    sorted_fragment_df = pd.DataFrame(fragment_df).sort_values(by="Length",ascending=True).reset_index()
    fitting_stats_intersection = []
    read_counts_list = [0 for i in range(len(sorted_fragment_df))]
    read_id_list = [[] for i in range(len(sorted_fragment_df))]
    for alignment_df_row_refstart, alignment_df_row_refend, alignment_df_row_id in tqdm(zip( alignment_df["Refstart"], alignment_df["Refend"],alignment_df["ID"]), total = alignment_df.shape[0]):
        ever_any_fit = 0
        for fragment_df_index, fragment_df_row_start, fragment_df_row_end, fragment_df_row_fragment in zip([i for i in range(sorted_fragment_df.shape[0])],sorted_fragment_df["Start"],sorted_fragment_df["End"],sorted_fragment_df["Fragment"]):
            if max(0,alignment_df_row_refstart) >= fragment_df_row_start and min(ref_data["Length"],alignment_df_row_refend) <= fragment_df_row_end:
                read_counts_list[fragment_df_index] = read_counts_list[fragment_df_index] + 1
                read_id_list[fragment_df_index].append(alignment_df_row_id)
                fitting_stats_intersection.append(fragment_df_row_fragment)
                ever_any_fit = 1
                break
        if ever_any_fit == 0:
            fitting_stats_intersection.append("")
    return read_id_list,read_counts_list,fitting_stats_intersection,sorted_fragment_df
...
def create_colored_bed(table_name = "", output_folder = output, output_file = "", sample_type = ""):
    ...
    bed_df = df[["Fragment","Start","End","Fragment","Score","Strand","Thikstart","Thikend","Colors"]]
    ...
    bed_df = most_abundant_df[["Fragment","Start","End","Fragment","Score","Strand","Thikstart","Thikend","Colors"]]
    bed_df.iloc[:,0] = "RNA45SN1"
    template = 'track name="ItemRGBDemo" description="Item RGB demonstration" visibility=2 itemRgb="On"'
...
def determine_general_cut_sites(alignment_df,output):
    ...
    for row_refstart, row_refend in tqdm(zip(alignment_df["Refstart"],alignment_df["Refend"]), total= alignment_df.shape[0]):
        start_sites[row_refstart-1] += 1
        end_sites[row_refend-1] += 1
    start_end_df = pd.DataFrame({"Starts":start_sites,"Ends": end_sites,"Base": sequence_list}, index = range(0,len(fasta_file.fetch(reference))))
    ...
    start_end_df["Starting_points"] = [1 if i > starts_threshold else 0 for i in start_end_df["Starts"]]
    start_end_df["End_points"] = [1 if i > ends_threshold else 0 for i in start_end_df["Ends"]]
    ...
    for i in tqdm(start_sites, total = len(start_sites)):
        temp_dict = {"Reference": "RNA45SN1","Start": i[0],"End": i[0],"Fragment": "Startcut","Score": i[1]/number_of_reads * 1000000,"Strand": ".","Thikstart": i[0] ,"Thikend": i[0],"Colors": "0,153,0" }
        start_sites_list.append(temp_dict)
    start_sites_bed = pd.DataFrame.from_dict(start_sites_list)
    start_sites_bed.to_csv(output + "start_sites_general.bed",sep="\t",header=False,index=False)
    for j in tqdm(end_sites, total = len(end_sites)):
        temp_dict = {"Reference": "RNA45SN1","Start": j[0],"End": j[0],"Fragment": "Endcut","Score": j[1]/number_of_reads * 1000000,"Strand": ".","Thikstart": j[0] ,"Thikend": j[0],"Colors": "0,153,153" }
        end_sites_list.append(temp_dict)
    end_sites_bed = pd.DataFrame.from_dict(end_sites_list)
    end_sites_bed.to_csv(output + "end_sites_general.bed",sep="\t",header=False,index=False)



def determine_fragment_based_cut_sites(alignment_df,fragment_df,output):
    cut_sides_df = fragment_df.loc[fragment_df["Fragment"].isin(["5ETS","ITS1","ITS2","3ETS","18S","5-8S","28S"])]
    start_sites = [0 for i in range(len(fasta_file.fetch(reference)))]
    end_sites = [0 for i in range(len(fasta_file.fetch(reference)))]
    sequence_list =[base for base in str(fasta_file.fetch(reference))]
    ...
    starts_means_over_reference_summ = [0 for i in range(len(fasta_file.fetch(reference)))]
    starts_std_over_reference_summ = [0 for i in range(len(fasta_file.fetch(reference)))]
    ...
    starts_threshold = [mean + (2 * std_dev) for mean,std_dev in zip(starts_means_over_reference_summ,starts_std_over_reference_summ)]
    ...
    ends_means_over_reference_summ = [0 for i in range(len(fasta_file.fetch(reference)))]
    ...
    ends_threshold = [mean + (2 * std_dev) for mean,std_dev in zip(ends_means_over_reference_summ,ends_std_over_reference_summ)]
    ...
    for i in tqdm(start_sites, total = len(start_sites)):
        ...
        start_sites_list.append(temp_dict)
    start_sites_bed = pd.DataFrame.from_dict(start_sites_list)
    start_sites_bed.to_csv(output + "start_sites_fragment_based.bed",sep="\t",header=False,index=False)

    for j in tqdm(end_sites, total = len(end_sites)):
        temp_dict = {"Reference": "RNA45SN1","Start": j[0],"End": j[0],"Fragment": "Endcut","Score": j[1]/number_of_reads * 1000000,"Strand": ".","Thikstart": j[0] ,"Thikend": j[0],"Colors": "0,153,153" }
        end_sites_list.append(temp_dict)
    end_sites_bed = pd.DataFrame.from_dict(end_sites_list)
    end_sites_bed.to_csv(output + "end_sites_fragment_based.bed",sep="\t",header=False,index=False)
...
fragment_df = fragment_df.sort_values(by=['Start','Length'], ascending=[True,True]).reset_index(drop=True)
...
temp_list = list()
ref_data = {"ID" : str(reference), "Sequence" : str(fasta_file.fetch(reference)), "Length": len(fasta_file.fetch(reference)), "Refstart": 0, "Refend": len(fasta_file.fetch(reference))}
...
with Pool(cores) as p:
    pool_output = p.starmap(percentage_of_fits_parrallel, zip(indices_list,read_list_df,repeat(reference_len),repeat(reference_seq),repeat(identity)))
selected_indices = []
...
for index,row in fragment_df.iterrows():
    ...
    new_fragment = new_fragment[:int(row["Start"])] + new_string + new_fragment[int(row["End"]):]
...
with Pool(cores) as p:
    pool_output = p.starmap(argmax_of_min_overlap_fragment_association_parallel, zip(alignment_df["Sequence"], alignment_df["Refstart"], alignment_df["Refend"], alignment_df["Length"],alignment_df["ID"],repeat(fragment_df)))
fitting_stats = [[] for i in range(0,len(fragment_df))]
...
logger.info("Associate querys to the shortest literature fragments extending the querys boundaries")
read_id_list,read_counts_list,fitting_stats_intersection,sorted_fragment_df = min_length_intersection_association(alignment_df,fragment_df)
...
alignment_df = alignment_df.hstack([pl.Series("Associated_Fragments_Overlap",fitting_stats_fragment)])
alignment_df = alignment_df.hstack([pl.Series("Associated_Fragments_Intersection",fitting_stats_intersection)])
...
sorted_fragment_df["n_Reads"] = read_counts_list
sorted_fragment_df["rel_n_Reads"] = sorted_fragment_df["n_Reads"] / number_of_reads
sorted_fragment_df["IDS"] = read_id_list
sorted_fragment_df.to_csv(f"{output}template_intersection_fragment_df.csv",sep=";")


over_zero_fragment_df = fragment_df.loc[fragment_df["n_Reads"] > 0]
bed_fragment_df = pd.DataFrame(over_zero_fragment_df[["Reference","Start","End","Fragment"]])
create_colored_bed(table_name = f"{output}template_fragment_df.csv", output_folder = output, output_file = "template_driven.bed", sample_type = sample_type)


over_zero_sorted_fragment_df = sorted_fragment_df.loc[sorted_fragment_df["n_Reads"] > 0]
sorted_bed_fragment_df = pd.DataFrame(over_zero_sorted_fragment_df[["Reference","Start","End","Fragment"]])    
create_colored_bed(table_name = f"{output}template_intersection_fragment_df.csv", output_folder = output, output_file = "template_driven_intersections.bed", sample_type = sample_type)
logger.info("DONE")
...
logger.info("Shortest fragment associated queries")
logger.info(sorted_fragment_df)        
