...
def plot_matrix(dbscan_matrix:np.array, color_sample:str, template_df_name:str, id_dict:dict):
    """
    ...
    color_dict = {}
    for sample,color in zip(["Nucleus","Cytoplasm","SN1","SN2","SN3"],["navy","darkred","seagreen","darkorange","darkviolet"]):
        color_dict[sample] = color
    for sample,color in zip(["blue","red","green","orange","purple"],["navy","darkred","seagreen","darkorange","darkviolet"]):
        color_dict[sample] = color   
    fig,ax = plt.subplots(1,1,figsize = (10,10), dpi=500)
    ax.scatter(x = start_points, y = end_points, alpha=alphas, s = 1, c=color_dict[color_sample])
    for template,start,end in zip(template_df["Fragment"], template_df["Start"], template_df["End"]):
        
        ax.hlines(y=end,xmin=start,xmax=end, color="red", linewidth=1, linestyles="dashed")
        ax.vlines(x=start,ymin=start,ymax=end, color = "red", linewidth=1, linestyles="dashed")
        ax.hlines(y=start,xmin=start,xmax=end, color="red", linewidth=1, linestyles="dashed")
        ax.vlines(x=end,ymin=start,ymax=end, color = "red", linewidth=1, linestyles="dashed")
        text_x_position = int(start+((end-start)*0.5)) - (len(template) * 50)
        text_y_position = start-250
        print(text_x_position,text_y_position)
...
alignment_df = pl.read_csv(alignment_df_name,separator=";")
