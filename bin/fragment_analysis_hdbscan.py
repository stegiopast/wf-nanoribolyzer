    ...
    for read in tqdm(samfile.fetch(),total=number_of_reads):
        ...
        for c_tuple in c_tuple_read: 
            ...
            if operation == 0:
                ...
                position_read += length
        #read_data = pl.DataFrame({"ID" : read.query_name,"Sequence" : string,"Proportion_Sequence": 0, "Length": int(read.reference_end - read.reference_start), "Refstart": read.reference_start, "Refend": read.reference_end, "n_Reads": 1,"IDS": [read.query_name]})
        #alignment_df = alignment_df.vstack(read_data)
...
def create_intensity_matrix(fusion_alignment_df):
    intensity_matrix = np.zeros((max(fusion_alignment_df["Refend"])+1,max(fusion_alignment_df["Refend"])+1),dtype=int)
    id_dict = {}
    for x,y,id in zip(fusion_alignment_df["Refstart"],fusion_alignment_df["Refend"],fusion_alignment_df["ID"]):
...
def hdbscan_clustering(fusion_alignment_df,dbscan_matrix,id_dict,min_number_of_neighbours):
...
def hdbscan_fusion(fusion_alignment_df = pl.DataFrame(), min_number_of_neighbours = 5):
        reference_dict = {"ID" : str(reference), "Sequence" : str(fasta_file.fetch(reference)),"Proportion_Sequence": [], "Length": len(fasta_file.fetch(reference)), "Refstart": 0, "Refend": len(fasta_file.fetch(reference)), "n_Reads": 1, "IDS": []}
        ...
        for i,row_sequence,rowstart,rowend in tqdm(zip(range(consensus_df.shape[0]),consensus_df["Sequence"],consensus_df["Refstart"],consensus_df["Refend"]),total = consensus_df.shape[0]):
...
final_df, single_reads_df  = hdbscan_fusion(alignment_df,min_neighbors)
...
create_colored_bed(table_name = f"{output}fragment_df.csv", output_folder = output, output_file = "no_template.bed", sample_type = sample_type)
